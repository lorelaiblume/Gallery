...
const DEFAULT_CONFIG = {
  ...
  fonts:  { heading: 'Sigmar One', nav: 'Cormorant Garamond' },
  colors: { bg: '#ffffff', text: '#111111', accent: '#111111', subheading: '#999999' }
};
...
async function loadSiteConfig() {
  try {
    const doc = await db.collection('siteConfig').doc('v2').get();
    if (doc.exists) {
      const data = doc.data();
      currentConfig = {
        ...DEFAULT_CONFIG,
        ...data,
        fonts:  { ...DEFAULT_CONFIG.fonts,  ...(data.fonts  || {}) },
        colors: { ...DEFAULT_CONFIG.colors, ...(data.colors || {}) },
        tabs:   data.tabs || DEFAULT_CONFIG.tabs
      };
    }
  } catch (e) {
...
function applyConfig(config) {
  ...
  root.style.setProperty('--color-subheading',  config.colors.subheading);
  root.style.setProperty('--font-heading', `'${config.fonts.heading}', cursive, serif`);
  root.style.setProperty('--font-nav',     `'${config.fonts.nav}', Georgia, serif`);

  document.title = config.title || 'Lorelai Blume';
  document.getElementById('siteTitle').textContent      = config.title     || '';
  document.getElementById('siteSubheading').textContent = config.subheading || '';

  renderNav(config.tabs);
}
...
function scheduleConfigSave() {
  ...
  configSaveTimer = setTimeout(async () => {
    try {
      await db.collection('siteConfig').doc('v2').set(currentConfig);
...
function enterEditMode() {
  ...
  siteSettingsBtn.classList.remove('hidden');
  if (currentCategory !== 'film' && currentCategory !== 'apps') {
...
function exitEditMode() {
  ...
  document.body.classList.add('view-mode');
  siteSettingsBtn.classList.add('hidden');
...
editLink.addEventListener('click', async e => {
  ...
  try {
    ...
    if (result.user.email === OWNER_EMAIL) {
      ...
      await auth.signOut();
      alert('Sorry, only Lorelai can edit this site!');
    }
  } catch (e) {
    console.error(e);
  }
...
function renderNav(tabs) {
  ...
  tabs.forEach(tab => {
    ...
    btn.className = 'nav-btn' + (tab.id === currentCategory ? ' active' : '');
...
function switchToTab(tabId) {
  ...
  document.querySelectorAll('.nav-btn').forEach(b => {
    b.classList.toggle('active', b.dataset.category === tabId);
  });
...
const HEADING_FONTS = [
  { name: 'Sigmar One',          label: 'Sigmar'    },
  { name: 'Playfair Display',    label: 'Playfair'  },
  { name: 'Bebas Neue',          label: 'Bebas'     },
  { name: 'DM Serif Display',    label: 'DM Serif'  },
...
const NAV_FONTS = [
  ...
  { name: 'Libre Baskerville',  label: 'Libre'     },
...
function openSiteEditor() {
  siteEditor.classList.add('open');
  populateSiteEditor();
}

function closeSiteEditor() {
  siteEditor.classList.remove('open');
}

siteSettingsBtn.addEventListener('click', () => {
  if (siteEditor.classList.contains('open')) {
    closeSiteEditor();
  } else {
    openSiteEditor();
  }
...
document.getElementById('seClose').addEventListener('click', closeSiteEditor);
...
function populateSiteEditor() {
  ...
  titleInput.oninput = () => {
    ...
    document.getElementById('siteTitle').textContent = titleInput.value;
    document.title = titleInput.value;
    scheduleConfigSave();
  };

  const subInput = document.getElementById('se-subheading');
  subInput.value = currentConfig.subheading;
  subInput.oninput = () => {
    currentConfig.subheading = subInput.value;
    document.getElementById('siteSubheading').textContent = subInput.value;
    scheduleConfigSave();
  };
  ...
  // Fonts
  renderFontGrid('se-heading-fonts', HEADING_FONTS, currentConfig.fonts.heading, name => {
    currentConfig.fonts.heading = name;
    document.documentElement.style.setProperty('--font-heading', `'${name}', cursive, serif`);
    scheduleConfigSave();
  });

  renderFontGrid('se-nav-fonts', NAV_FONTS, currentConfig.fonts.nav, name => {
    currentConfig.fonts.nav = name;
    document.documentElement.style.setProperty('--font-nav', `'${name}', Georgia, serif`);
    scheduleConfigSave();
  });

  // Colors
  setupColorPicker('se-color-bg', 'bg', '--color-bg');
  setupColorPicker('se-color-text', 'text', '--color-text');
  setupColorPicker('se-color-accent', 'accent', '--color-accent');
  setupColorPicker('se-color-subheading', 'subheading', '--color-subheading');
}

function renderTabsEditor() {
  const list = document.getElementById('se-tabs-list');
  list.innerHTML = '';
  const tabs = currentConfig.tabs;

  tabs.forEach((tab, i) => {
    const row = document.createElement('div');
    row.className = 'se-tab-row';

    // Up/down arrows
    const arrows = document.createElement('div');
    arrows.className = 'se-tab-arrows';

    const upBtn = document.createElement('button');
    upBtn.className = 'se-tab-arrow';
    upBtn.textContent = '▲';
    upBtn.title = 'Move up';
    upBtn.disabled = i === 0;
    upBtn.addEventListener('click', () => {
      ...
      scheduleConfigSave();
      renderTabsEditor();
      renderNav(currentConfig.tabs);
    });

    const downBtn = document.createElement('button');
    downBtn.className = 'se-tab-arrow';
    downBtn.textContent = '▼';
    ...
    input.className = 'se-tab-label-input';
    input.value = tab.label;
    input.addEventListener('input', () => {
      tab.label = input.value;
      renderNav(currentConfig.tabs);
      scheduleConfigSave();
    });
    ...
    del.className = 'se-tab-del';
    del.textContent = '×';
    del.title = 'Remove tab';
    del.addEventListener('click', () => {
      if (tabs.length <= 1) return; // keep at least one
      const wasActive = currentCategory === tab.id;
      tabs.splice(i, 1);
      if (wasActive && tabs.length > 0) switchToTab(tabs[0].id);
      scheduleConfigSave();
      renderTabsEditor();
      renderNav(currentConfig.tabs);
    });

    row.append(arrows, input, del);
    list.appendChild(row);
  });
}

document.getElementById('seAddTab').addEventListener('click', () => {
  const id = 'tab_' + Date.now();
  currentConfig.tabs.push({ id, label: 'New Tab' });
  renderTabsEditor();
  renderNav(currentConfig.tabs);
  scheduleConfigSave();
  switchToTab(id);
...
function renderFontGrid(containerId, fonts, currentFont, onSelect) {
  const el = document.getElementById(containerId);
  el.innerHTML = '';
  fonts.forEach(f => {
    const btn = document.createElement('button');
    btn.className = 'se-font-btn' + (f.name === currentFont ? ' active' : '');
    btn.style.fontFamily = `'${f.name}', serif`;
    btn.textContent = f.label;
    btn.addEventListener('click', () => {
      onSelect(f.name);
      renderFontGrid(containerId, fonts, f.name, onSelect);
    });
    el.appendChild(btn);
  });
}

function setupColorPicker(inputId, colorKey, cssVar) {
  const input = document.getElementById(inputId);
  input.value = currentConfig.colors[colorKey];
  input.oninput = () => {
    currentConfig.colors[colorKey] = input.value;
...
function openFilmPlayer(film) {
  ...
  commentaryUnsubscribe = db.collection('films').doc(film.id)
    ...
    .onSnapshot(snapshot => {
      ...
      document.getElementById('commentaryToggleLabel').classList.toggle('hidden', allCommentary.length === 0);
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') {
    if (!document.getElementById('filmModal').classList.contains('hidden')) closeFilmPlayer();
    if (!document.getElementById('appModal').classList.contains('hidden'))  closeAppModal();
  }
});
...
function checkCommentary(video) {
  ...
  const toggleCheckbox = document.getElementById('commentaryToggle');
  const enabled = !toggleCheckbox || toggleCheckbox.checked;
...
function setupFilmEditor(film, video) {
...
function startShapeAnimation(stage) {
  ...
  const regions = [
    { color: 0x1A4FCC, shape: blobShape(-1.52, 0.82, 0.40, 0.34, [0.2, 0.05, 0.28, -0.12, 0.18, -0.22, 0.08, -0.18, 0.24, -0.08]), z: 0.35 },
    { color: 0x55CCEE, shape: makeShape([[-2,1.33],[-1.05,1.33],[-0.52,0.62],[-0.92,0.0],[-1.65,-0.32],[-2,-0.08]]), z: -0.4 },
    { color: 0x88EE28, shape: makeShape([[-2,0.82],[-1.18,1.33],[-0.28,1.33],[0.12,0.78],[-0.08,0.22],[-0.82,-0.02],[-1.78,0.28]]), z: 0.05 },
    { color: 0xFF7D18, shape: makeShape([[-1.75,0.42],[-0.48,0.98],[0.22,1.33],[0.88,1.08],[0.62,0.38],[0.02,-0.18],[-0.78,0.02],[-1.38,0.18]]), z: -0.1 },
    { color: 0xE82E1E, shape: makeShape([[-0.58,0.42],[0.18,0.85],[0.72,0.60],[0.52,-0.22],[0.08,-0.72],[-0.38,-0.88],[-0.78,-0.28],[-0.68,0.08]]), z: 0.2 },
    { color: 0xFF0E88, shape: makeShape([[0.22,1.33],[0.88,1.33],[0.82,-1.33],[0.08,-1.33]]), z: 0.0 },
    { color: 0xFFB8C5, shape: blobShape(-0.72, -0.52, 0.72, 0.60, [0.12,-0.18,0.16,-0.08,0.22,-0.14,0.06,-0.22,0.14,-0.06,0.20,-0.16]), z: 0.45 },
    { color: 0xFF6644, shape: makeShape([[-2,-0.32],[-0.08,-0.22],[0.08,-1.33],[-2,-1.33]]), z: -0.3 },
    { color: 0xC8B2FF, shape: makeShape([[0.88,1.33],[2,1.33],[2,0.12],[1.52,-0.12],[0.92,0.12],[0.82,0.62]]), z: 0.0 },
    { color: 0x9968EE, shape: makeShape([[1.12,-0.12],[2,0.12],[2,-0.52],[1.52,-0.78],[0.88,-0.52]]), z: 0.18 },
    { color: 0x8898EE, shape: makeShape([[0.88,-0.52],[1.52,-0.78],[2,-0.52],[2,-1.18],[1.12,-1.08],[0.62,-0.88]]), z: -0.12 },
    { color: 0xAAFF88, shape: makeShape([[1.12,-1.08],[2,-1.18],[2,-1.33],[0.92,-1.33]]), z: 0.28 },
    { color: 0xDDFF88, shape: makeShape([[0.62,-1.15],[1.12,-1.08],[0.92,-1.33],[0.52,-1.33]]), z: 0.08 }
  ];
  ...
  regions.forEach((r, i) => {
    ...
    const mat  = new THREE.MeshPhongMaterial({ color: r.color, side: THREE.DoubleSide, shininess: 70, specular: new THREE.Color(0x333333) });
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.z  = r.z;
    mesh.userData    = { origZ: r.z, phase: i * 0.71, ry: 0.18 + (i % 5) * 0.055 };
    scene.add(mesh);
    meshes.push(mesh);
  });
...
(async () => {
  ...
  if (currentConfig.tabs.length > 0) {
    switchToTab(currentConfig.tabs[0].id);
  }
})();
